...
function init_logfile() {
  local log="/tmp/pi-installer.log"
  {
    echo -e "#================================================================#"
    echo -e "# New pi-Installer session started on: $(date) #"
    echo -e "#================================================================#"
...
#================================================#
#=========== pi-installer SETTINGS ==============#
#================================================#

function read_pi-installer_ini() {
  local func=${1}

  if [[ ! -f ${INI_FILE} ]]; then
    log_warning "Reading from .pi-installer.ini failed! File not found! Creating default ini file."
    init_ini
  fi

  log_info "Reading from .pi-installer.ini ... (${func})"
  source "${INI_FILE}"
}

function init_ini() {
  ### remove pre-version 4 ini files
  if [[ -f ${INI_FILE} ]] && ! grep -Eq "^# pi-installer v4\.0\.0$" "${INI_FILE}"; then
    rm "${INI_FILE}"
  fi
  ...
  if [[ ! -f ${INI_FILE} ]]; then
    {
      ...
      echo -e "#=================================================#"
      echo -e "# pi-installer - Installation And Update Helper   #"
      echo -e "#                                                 #"
